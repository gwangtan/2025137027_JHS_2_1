...
public class TurnBasedGame : MonoBehaviour
{
    ...
    [Header("아이템 개수")]
    public int rareWeaponCount = 0;
    public int rareArmorCount = 0;


    public void UpdateRareTotalUI()
    {
        // 1. 아이템 총합 계산
        int totalRareItems = rareWeaponCount + rareArmorCount;
        totalText.text = $"현재 턴: {turn} \n총 레어 아이템: {totalRareItems}개";

        
        Debug.Log($"UI 업데이트 - Turn: {turn}, Total Items: {totalRareItems}");
    }
    ...
    void SimulateTurn()
    {
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            if (totalDamage >= enemyHP)
            {
                ...
                if (reward == "Weapon" && Random.value < 0.2f + bturn)
                {
                    rareItemObtained = true;
                    rareWeaponCount++; // 1. 레어 무기 개수 증가
                    UpdateRareTotalUI(); // 2. UI 업데이트 함수 호출
                    Debug.Log("레어 무기 획득!");
                }
                else if (reward == "Armor" && Random.value < 0.2f + bturn)
                {
                    rareItemObtained = true;
                    rareArmorCount++;  // 1. 레어 방어구 개수 증가
                    UpdateRareTotalUI(); // 2. UI 업데이트 함수 호출
                    Debug.Log("레어 방어구 획득");
                }
            }
